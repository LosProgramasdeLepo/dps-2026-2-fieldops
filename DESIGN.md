...
**Replanificación.** `Replanner` arma la alternativa sobre el agregado: cancelar actividad, correr ventanas (`delay`) y soltar asignaciones inválidas. Después rellena huecos con el sugeridor. Cancelar y atrasar tocan el itinerario: si no está en `DRAFT`, `returnToDraft` y recién ahí. Reemplazar indisponibles también en `IN_REVIEW`, sin volver a borrador. Cancelar un predecesor suelta esa dependencia en las que lo usaban; no borra las actividades dependientes. `delay` corre la actividad y empuja dependientes (por el grafo, no por el orden de la lista) lo justo para que el predecesor termine antes; si alguna ventana quedaría fuera del período, no aplica nada.
